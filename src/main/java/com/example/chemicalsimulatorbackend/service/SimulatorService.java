...
public class SimulatorService {
    ...
    // Словник, який зберігає окремий стан симулятора для кожного користувача
    private final Map<String, SimulatorState> sessions = new ConcurrentHashMap<>();

    // Метод для отримання або створення нової симуляції за ID
    public SimulatorState getOrCreateState(String sessionId) {
        if (sessionId == null || sessionId.isEmpty()) {
            sessionId = "default_session"; // Запобіжник
        }
        return sessions.computeIfAbsent(sessionId, k -> new SimulatorState());
    }

    public SimulatorState getStatus(String sessionId) {
        return getOrCreateState(sessionId);
    }

    public void startProcess(String sessionId, String recipe) {
        getOrCreateState(sessionId).startProcess(recipe);
    }

    public void togglePause(String sessionId) {
        getOrCreateState(sessionId).togglePause();
    }

    public void resetProcess(String sessionId) {
        getOrCreateState(sessionId).resetProcess();
    }

    public void processTick(String sessionId) {
